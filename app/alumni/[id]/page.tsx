...
export default async function AlumniDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell><PageHeader title="" backHref="/alumni" />
    <section className="relative overflow-hidden rounded-[28px] bg-[linear-gradient(145deg,#31594e,#234239)] p-6 text-white shadow-[0_18px_40px_rgba(42,79,69,0.22)]">
      <div className="flex items-start gap-4"><div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full border border-white/25 bg-white/12 font-serif text-3xl font-bold">{person.name.slice(0, 1)}</div><div className="pt-1"><h1 className="font-serif text-2xl font-bold">{person.name}</h1><p className="mt-1 text-xs text-white/65">{person.graduationYear} 届 · {person.className || "溆浦一中校友"}</p><p className="mt-3 text-sm font-semibold">{person.university}</p><p className="mt-1 text-xs text-white/70">{person.major} · {person.city}</p></div></div>
